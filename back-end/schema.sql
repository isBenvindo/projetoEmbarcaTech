...
-- Índices
-- contagens_pizzas
CREATE INDEX IF NOT EXISTS idx_contagens_timestamp ON contagens_pizzas(timestamp);
CREATE INDEX IF NOT EXISTS idx_contagens_timestamp_date ON contagens_pizzas(DATE(timestamp));
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_views WHERE viewname = 'contagens_por_hora'
  ) THEN
    CREATE VIEW contagens_por_hora AS
    SELECT 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_views WHERE viewname = 'contagens_por_dia'
  ) THEN
    CREATE VIEW contagens_por_dia AS
    SELECT 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_views WHERE viewname = 'estatisticas_hoje'
  ) THEN
    CREATE VIEW estatisticas_hoje AS
    SELECT 
        COUNT(*) as total_contagens,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_views WHERE viewname = 'ultimas_contagens_24h'
  ) THEN
    CREATE VIEW ultimas_contagens_24h AS
    SELECT 
        id,
        timestamp,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_views WHERE viewname = 'velocidade_producao'
  ) THEN
    CREATE VIEW velocidade_producao AS
    SELECT 
        DATE_TRUNC('hour', timestamp) as hora,
...
BEGIN
    ...
    SELECT 
        ...
        EXTRACT(EPOCH FROM (DATE_TRUNC('hour', c.timestamp) + 
        (EXTRACT(MINUTE FROM c.timestamp)::INTEGER / intervalo_minutos) * 
        (intervalo_minutos || ' minutes')::INTERVAL))::BIGINT as timestamp_unix
...
COMMENT ON TABLE contagens_pizzas IS 'Tabela existente: pizzas detectadas';
...
COMMENT ON VIEW contagens_por_hora IS 'View para gráficos de contagens por hora no Grafana';
COMMENT ON VIEW contagens_por_dia IS 'View para gráficos de contagens por dia no Grafana';
COMMENT ON VIEW velocidade_producao IS 'View para análise de velocidade de produção';
